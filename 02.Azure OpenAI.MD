...
* Initialize Azure AI Search resource - ``` ai init search ``` -> Follow the prompts to select your Azure subscription and Azure AI Search resource.
    1. See the persisted config from ai init search
        1. Get search endpoint - ``` ai config @search.endpoint ```
        1. Get search key -  ``` ai config @search.key ```
...
* Create or update your Azure AI Search index
    ...
    1. Use the search index in chat completions - ``` ai chat --user "What is the capital of France?" --index MyFiles ```
